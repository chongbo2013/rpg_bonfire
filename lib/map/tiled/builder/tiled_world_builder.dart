...
class TiledWorldBuilder {
  ...
  void _addImageLayer(ImageLayer layer) {
    ...
    final layerIsAbove = layer.properties
        ?.where(
          (element) =>
      element.name == 'above',
    )
        ...
        false;
    //是否需要视差
    final layerIsNoParallax = layer.properties
        ?.where(
          (element) =>
      element.name == 'layerIsNoParallax',
    )
        .isNotEmpty ??
        false;
    _components.add(
      BackgroundImageGame(
        ...
        layerIsAbove:layerIsAbove,
          layerIsNoParallax:layerIsNoParallax
